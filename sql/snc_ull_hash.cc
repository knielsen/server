...
static void free_ull_entry(void *v)
{
  Snc_ull *e= (Snc_ull *)v;
  if (!e || !e->alloced_from)
    return;
...
static const uchar *get_key(const void *e, size_t *len,
                                  my_bool)
{
    if (!e)
    {
    ...
    *len = ull_e->lock_name_len;
    return (const uchar*)ull_e->lock_name;
}

static const uchar *get_session_key(const void *e, size_t *len,
                                  my_bool)
{
    if (!e)
    {
...
void Snc_ull_hash::init()
{
  ...
  my_hash_init(PSI_INSTRUMENT_ME, &h, &my_charset_bin, ULL_HASH_SIZE,
               0, 0, get_key, free_ull_entry, HASH_UNIQUE);
  cur_root = &root1;
  cur_total_alloc= 0;
  old_root = &root2;
  old_total_alloc= 0;
  ...
  my_hash_init(PSI_INSTRUMENT_ME, &session_h, &my_charset_bin, ULL_SESSION_HASH_SIZE,
               0, 0, get_session_key, 0, HASH_UNIQUE);
  inited = 1;
...
void Snc_ull_hash::try_reset_root(MEM_ROOT* root, size_t *total_alloc)
{
...
{
  init_alloc_root(PSI_INSTRUMENT_ME, root, ULL_ROOT_BLOCK_SIZE, 0, MYF(0));
}
                ...
                uint lock_name_len)
{
  ...
  if (cur_total_alloc > memory_target)
  {
    try_reset_root(old_root, &old_total_alloc);
    if (old_total_alloc < memory_target)
    {
      MEM_ROOT* tmp = cur_root;
      size_t tmp2= cur_total_alloc;
      cur_root = old_root;
      cur_total_alloc= old_total_alloc;
      old_root = tmp;
      old_total_alloc= tmp2;
    }
  }

  Snc_ull* ull_e = (Snc_ull*)alloc_root(cur_root, mem_size);
  cur_total_alloc+= mem_size;
...
void Snc_ull_hash::show_pool(SHOW_VAR* var, char* buf, bool old)
{
    ...
    var->value = buf;
    *(longlong*)buf = (longlong)(old ? old_total_alloc : cur_total_alloc);
}
