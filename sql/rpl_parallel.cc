...
rpl_parallel_print_info(THD *thd, THD *conflicting_thd)
{
  ...
  if (ha_list && ha_list->is_started())
  {
    ...
    do
    {
      transaction_participant *engine= ha_list->ht();
      if (engine->show_status)
        (*engine->show_status)(static_cast<handlerton *>(engine), thd,
                               rpl_parallel_print_engine, HA_ENGINE_STATUS);
  ...
  more_events:
    for (qev= events; qev; qev= next_qev)
    {
      ...
      if ((event_type= qev->ev->get_type_code()) == GTID_EVENT)
      {
        ...
        DBUG_EXECUTE_IF("rpl_parallel_delay_gtid_7_x_100x_start", {
            if (rgi->current_gtid.domain_id==7 &&
                rgi->current_gtid.seq_no % 100 == 0)
              my_sleep(10000);
          });
#ifdef ENABLED_DEBUG_SYNC
        DBUG_EXECUTE_IF("hold_worker_on_schedule", {
            if ((rgi->current_gtid.domain_id == 0 || rgi->current_gtid.domain_id == 100) &&
                rgi->current_gtid.seq_no == 100) {
                  debug_sync_set_action(thd,
                                          ...
                                          uint64_t sub_id)
{
  int err= 0;
  ooo_dependency *d;
  if (!(d= (ooo_dependency *)my_malloc(PSI_INSTRUMENT_ME, sizeof(*d),
                                       MYF(MY_WME))))
    return 1;
  d->gtid= *gtid;
  d->sub_id= sub_id;
...
rpl_parallel::find(uint32 domain_id, Relay_log_info *rli)
{
                                                ...
                                                sizeof(domain_id))))
  {
    ...
    */
    ...
    my_hash_init(PSI_INSTRUMENT_ME, &e->ooo_dependency_hash, &my_charset_bin, 4,
